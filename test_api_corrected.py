...
def test_api():
    ...
    if token:
        ...
        # Test gamification endpoints (corrected URLs)
        try:
            response = requests.get(f"{BASE_URL}/user_points/1", headers=headers)
            if response.status_code == 200:
                points = response.json()
                print(f"✅ Gamification user points endpoint working - Points: {points}")
            else:
                print(f"❌ Gamification user points endpoint failed: {response.status_code}")
        except Exception as e:
            print(f"❌ Gamification user points endpoint error: {e}")

        # Test reports endpoints (corrected URLs)
        try:
            response = requests.get(f"{BASE_URL}/sales_monthly", headers=headers)
            if response.status_code == 200:
                sales = response.json()
                print(f"✅ Monthly sales report endpoint working - Data: {sales}")
            else:
                print(f"❌ Monthly sales report endpoint failed: {response.status_code}")
        except Exception as e:
