...
def test_api():
    ...
    if token:
        ...
        try:
            response = requests.get(f"{BASE_URL}/gamification/points", headers=headers)
            if response.status_code == 200:
                points = response.json()
                print(f"✅ Gamification points endpoint working - Points: {points}")
            else:
                print(f"❌ Gamification points endpoint failed: {response.status_code}")
        except Exception as e:
            print(f"❌ Gamification points endpoint error: {e}")

        # Test reports endpoints
        try:
            response = requests.get(f"{BASE_URL}/reports/monthly-sales", headers=headers)
            if response.status_code == 200:
                sales = response.json()
